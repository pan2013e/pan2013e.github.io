...
$ \displaystyle\sum^{\infty}_{k=0}ka_kx^k=x\sum^\infty_{k=0}a_k(x^k)'=x(\sum^{\infty}_{k=0}a_kx^k)':=xf'(x) $

柯西乘积:$ \displaystyle (\sum^{\infty}_{k=0}b_kx^k(\sum^{\infty}_{k=0}a_kx^k):=f(x)g(x)=\sum^{\infty}_{k=0}(\sum^{k}_{j=0}a_jb_{k-j})x^k $
...
$\displaystyle a_n=2a_{n-1}+3a_{n-2}+4^n+6,a_0=20,a_1=60. $

首先两边同乘$x^n$,然后作和，注意求和时变量的起始值不能让数列的下标越界，有

$\displaystyle \sum^\infty_{n=2}a_nx^n=2\sum^\infty_{n=2}a_{n-1}x^n+3\sum^\infty_{n=2}a_{n-2}x^n+\sum^\infty_{n=2}4^nx^n+6\sum^\infty_{n=2}x^n $
